...
import React, { useState, lazy, Suspense } from "react";
import LazyOnViewPreload from "../../../components/common/LazyOnViewPreload";
...
const preloadFeatures = () => import("./sections/FeaturesSection");
const preloadCTA = () => import("./sections/CTASection");

export default function LandingPage() {
  const [modalConfig, setModalConfig] = useState({ isOpen: false, role: "student" });
  const openLogin = (role) => setModalConfig({ isOpen: true, role });
  const closeLogin = () => setModalConfig((prev) => ({ ...prev, isOpen: false }));

  return (
    <div className="min-h-screen">
      {/* Hero */}
      <Suspense fallback={<HeroSkeleton />}>
        <HeroSection openLogin={openLogin} modalConfig={modalConfig} closeLogin={closeLogin} />
      </Suspense>

      {/* Features (chỉ mount khi scroll gần) */}
      <LazyOnViewPreload
